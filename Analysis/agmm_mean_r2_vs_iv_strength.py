...
for tau in tau_list:
    ...
    plt.errorbar(
        sub["iv_strength"],
        sub["avg_R2fin"],      # mean R2
        yerr=sub["std_R2fin"], # std of R2
        marker="o",
        capsize=4,
        linewidth=2,
        label=tau
...
plt.ylabel(r"Mean final $R^2$")
plt.title(r"AGMM: Mean $R^2$ versus IV strength")
plt.legend(title="Structural function")
plt.tight_layout()

# Save figure
output_path = project_root / "Analysis" / "agmm_mean_r2_vs_iv_strength.png"
