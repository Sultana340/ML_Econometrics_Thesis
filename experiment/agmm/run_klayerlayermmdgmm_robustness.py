...
def main():
    device = "cpu"
    print("Running KernelLayerMMDGMM robustness check on", device)
    ...
    # Robustness dimension for KernelLayerMMDGMM
    g_features_list = [5, 10, 20]

    # Robustness checks can use 3 MC runs to save CPU time
    monte_carlo = 3

    estimator = "KernelLayerMMDGMM"
    ...
    raw_path = os.path.join(
        results_dir,
        "robustness_kernellayer_gfeatures_raw_abs_iv06.csv"
    )

    summary_path = os.path.join(
        results_dir,
        "robustness_kernellayer_gfeatures_summary_abs_iv06.csv"
    )

    # --------------------------------------------------
    # Run experiments
    # --------------------------------------------------
    for tau_fn, iv_strength, dgp, num_data, g_features in settings:
        print("\n" + "=" * 60)
        print("Settings")
        print("=" * 60)
        print("tau_fn:", tau_fn)
        ...
        # --------------------------------------------------
        # Summary for this g_features setting
        # --------------------------------------------------
